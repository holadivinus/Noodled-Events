...
public class UltNoodleDataInPoint : VisualElement
{
    ...
    private void setupInternal(UltNoodleNodeUI node, NoodleDataInput input)
    {
        ...
        // Data/Value types
        if (input.Type == typeof(int)) ((newField = new IntegerField("") { value = input.DefaultIntValue }) as INotifyValueChanged<int>).RegisterValueChangedCallback(c => {input.DefaultIntValue = c.newValue; d(); }); else
        if (input.Type == typeof(float)) ((newField = new FloatField("") { value = input.DefaultFloatValue }) as INotifyValueChanged<float>).RegisterValueChangedCallback(c => {input.DefaultFloatValue = c.newValue; d(); }); else
        if (input.Type == typeof(string)) ((newField = new TextField("") { value = input.DefaultStringValue }) as INotifyValueChanged<string>).RegisterValueChangedCallback(c => {input.DefaultStringValue = c.newValue; d(); }); else
        if (input.Type == typeof(Vector2)) ((newField = new Vector2Field("") { value = input.DefaultVector2Value }) as INotifyValueChanged<Vector2>).RegisterValueChangedCallback(c => {input.DefaultVector2Value = c.newValue; d(); }); else
        if (input.Type == typeof(Vector3)) ((newField = new Vector3Field("") { value = input.DefaultVector3Value }) as INotifyValueChanged<Vector3>).RegisterValueChangedCallback(c => {input.DefaultVector3Value = c.newValue; d(); }); else
        if (input.Type == typeof(Vector4)) ((newField = new Vector4Field("") { value = input.DefaultVector4Value }) as INotifyValueChanged<Vector4>).RegisterValueChangedCallback(c => {input.DefaultVector4Value = c.newValue; d(); }); else
        if (input.Type == typeof(Quaternion)) ((newField = new Vector4Field("") { value = input.DefaultVector4Value }) as INotifyValueChanged<Vector4>).RegisterValueChangedCallback(c => {input.DefaultVector4Value = c.newValue; d(); }); else
        if (input.Type == typeof(bool)) ((newField = new Toggle("") { value = input.DefaultBoolValue }) as INotifyValueChanged<bool>).RegisterValueChangedCallback(c => {input.DefaultBoolValue = c.newValue; d(); }); else
        if (input.Type.Type.IsEnum) ((newField = new EnumField((Enum)Enum.ToObject(input.Type, input.DefaultIntValue))) as INotifyValueChanged<Enum>).RegisterValueChangedCallback((e) => {input.DefaultIntValue = Convert.ToInt32(e.newValue); d(); }); else
        {
            // custom object
        }
